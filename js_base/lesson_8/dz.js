...
alert(a);
// Как по мне, так самый интересный пример из всех в этом задании. Вначале подумал, что будет выведена 1,
// так как решил, что переменной a не будет существовать, пока мы не попадём внутрь блока if. Но нет...
// А оказывается переменная a, объявляется при синаксическом анализе. И по итогу перед условием уже существует
...
// Тут тоже не сразу понял, что произойдёт.
